...
describe('icon-only product surface', () => {
  ...
  it('keeps standard vectorization on the vtracer pipeline', () => {
    ...
    expect(worker).toContain('recoverOpaqueMattePaletteFringes');
    expect(worker).toContain('settings.matteReconstructed');
    expect(worker).toContain('resolveMatteFilterSpeckle');
    expect(worker).toContain('resolveMatteRasterSpeckleArea');
    expect(worker).toContain('absorbSmallPaletteComponents');
    expect(worker).toContain('morphCloseAlpha');
    expect(worker).toContain('upscaleImageDataSmooth');
    expect(worker).not.toContain('mattePipelineVersion');
    expect(worker).not.toContain('usesReconstructedMattePipeline');
    ...
    expect(route).toContain('PathSimplifyMode.Polygon');
    expect(route).toContain('PathSimplifyMode.Spline');
    expect(route).not.toContain('mattePipelineVersion');
